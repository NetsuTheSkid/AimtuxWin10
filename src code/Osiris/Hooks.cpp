...
Hooks::Hooks() noexcept
{
    ...
    if (GetWindowsDirectoryA(buffer, MAX_PATH))
        io.Fonts->AddFontFromFileTTF(strcat(buffer, "/Fonts/Tahoma"), 16.0f);
    ...
    **reinterpret_cast<void***>(memory.reset) = reinterpret_cast<void*>(hookedReset);
    ...
    interfaces.gameUI->messageBox("Netsu Owns You", "Aimtux for Windows has been successfully loaded.");
}
